        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-xl sm:rounded-lg">
                <div class="flex items-center justify-end bg-gray-50 px-4 py-3 text-right shadow sm:px-6">
                    <x-button href="{{ route('carousel.create') }}">{{ __('Cadastrar') }}</x-button>
                </div>
                            ...
                            <tbody>
                                @foreach ($carousels as $carousel)
                                    <tr class="border-b">
                                        ...
                                        <td class="truncate py-4 px-4">
                                            {{ $carousel->updated_at->format('d/m/Y') }}
                                        </td>
                                        <td class="truncate py-4 px-4">
                                            <x-button href="{{ route('carousel.edit', ['id' => $carousel->id]) }}" class="mb-1 lg:mb-0">Editar</x-button>

                                            <x-button href="{{ route('carousel.delete.do', ['id' => $carousel->id]) }}">Deletar</x-button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
